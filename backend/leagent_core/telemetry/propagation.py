...
All cross-process transports in ``leagent_core`` (:mod:`queue`,
:mod:`events`) stash a ``traceparent`` string on each message. This module
gives service code a consistent way to read/inject/restore that context so a
single logical operation shows up as one trace in Jaeger/Tempo.
"""
