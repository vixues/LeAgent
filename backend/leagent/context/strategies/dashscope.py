"""DashScope / Qwen-optimized context construction strategies.

Implements context ordering and structuring techniques aligned with the
Qwen model architecture:

- **Prefix caching**: DashScope returns ``prompt_tokens_details.cached_tokens``
  when request prefixes match. Stable system prompts (layers L0-L4) should
  stay deterministic across turns to maximize cache hits.
- **Boundary attention**: high-priority content placed at context boundaries
  (start/end) gets better attention; stable content in the middle is
  cache-friendly.
- **128K / 1M context windows**: Most Qwen3 models support 128 000 tokens;
  ``qwen-long`` supports up to 1 000 000.
"""
...
QWEN_DEFAULT_CONTEXT_WINDOW = 128_000
QWEN_LONG_CONTEXT_WINDOW = 1_000_000
...
def _tokenizer() -> Any | None:
    try:
        ...
        logger.debug("tiktoken_unavailable_for_dashscope_context", exc_info=exc)
        return None
...
class DashScopeContextStrategy:
    """Optimizes context layout for Qwen model attention patterns and
    DashScope's automatic prefix caching.
    ...
    2. **Stable middle**: rarely-changing content (system persona, policies,
       user preferences) — maximizes prefix reuse for DashScope's automatic
       cache (reflected in ``prompt_tokens_details.cached_tokens``).
    ...
    For long-context workflows, pass ``model_context_window=QWEN_LONG_CONTEXT_WINDOW``
    ...
    def compute_budgets(self, model_context_window: int | None = None) -> dict[str, int]:
        ...
        :data:`QWEN_LONG_CONTEXT_WINDOW`), budgets scale proportionally.
        """
    ...
    def order_blocks(self, blocks: list[ContextBlock]) -> list[ContextBlock]:
        ...
        - **EARLY-MIDDLE**: Stable facts, user preferences (cache prefix)
        - **LATE-MIDDLE**: Tool history, working set (moderate attention)
        - **END**: Most recent conversation turns (high attention)

        Keeping stable content in the middle also maximizes DashScope's
        automatic prefix cache reuse across requests.
        """
