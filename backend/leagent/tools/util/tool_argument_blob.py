...
class ToolArgumentBlobTool(BaseTool):
    ...
    description = (
        "Stage large UTF-8 text outside tool-call JSON: `action=create` returns a "
        "`blob_id`; `action=append` adds data via `chunk` or `chunk_base64` (prefer "
        "base64 for HTML/JSX); `action=finalize` seals the blob (required). "
        "Shortcut: `action=create_and_finalize` does create+append+finalize in one "
        "call (no `blob_id` needed — one is generated and returned). "
        "Then pass "
        "`source_blob_id` / `content_blob_id` / `diff_blob_id` / "
        "`html_blob_id` / `html_files_blob_id` / `old_string_blob_id` / `new_string_blob_id` into `code_execution`, "
        "`project_write`, `project_apply_patch`, `canvas_publish`, or `project_edit` instead of "
        "embedding megabyte strings in JSON."
    )
