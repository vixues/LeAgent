...
class CanvasPublishTool(BaseTool):
    ...
    description = (
        ...
        "`canvas_routing` (charts, dashboards, poster/slide frames — not plain Q&A), prefer "
        ...
        "Inline `html` is reliable under ~20KB tool-call JSON (soft budget under "
        "default output tokens); larger pages use `tool_argument_blob` → "
        "`html_blob_id` / `html_files_blob_id` or `html_paths` (store limit is "
        "canvas max_html_bytes, default 4MB — not 20KB). "
