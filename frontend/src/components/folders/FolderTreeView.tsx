...
interface FolderTreeViewProps {
  ...
  /** Header title above the tree. Defaults to `folders.showTree`. */
  title?: string;
  /** Root row label. Defaults to `folders.allFiles`. */
  rootLabel?: string;
  /** Empty-tree hint. Defaults to `folders.emptyTree`. */
  emptyLabel?: string;
}
      ...
      >
        <button
          ...
          aria-label={
            hasChildren
              ? isExpanded
                ? t('folders.treeCollapse')
                : t('folders.treeExpand')
              : undefined
          }
        >
          {hasChildren ? (
    ...
    <div className="flex flex-col h-full min-h-0">
      ...
      <div className="flex-1 min-h-0 overflow-y-auto py-2 px-2" role="tree">
        ...
        >
          ...
          />
          <span>{rootLabel ?? t('folders.allFiles')}</span>
        </div>
        ...
        {tree.length === 0 && (
          <div className="px-3 py-6 text-center text-sm text-muted-foreground-tertiary">
            {emptyLabel ?? t('folders.emptyTree')}
          </div>
        )}
      </div>
    </div>
